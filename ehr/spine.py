...
class Register_patient(Role):
    ...
    def onDeactivate(self, subject):
        ...
        hasActivated -= { (subj, role) for (subj, role) in hasActivated if role.name == 'Conceal_request' and pi7_1(role.what) == self.pat }
...
class Request_third_party_consent(Role):
    ...
    def onDeactivate(self, subject):
        ...
        hasActivated -= { (subj, role) for (subj, role) in hasActivated if subj == self.x and role.name == 'Third_party' and other_third_party_consent_requests(subject, self.x) == 0 }
        
        # S2.2.16 -- deactive Third-party-consent(x, pat, id):
        hasActivated -= { (subj, role) for (subj, role) in hasActivated if subj == self.x and role.name == 'Third_party_consent' and role.x == self.x and role.pat == self.pat and role.id == self.id and other_third_party_consent_requests(subject, role.x) == 0 }
...
class Request_consent_to_treatment(Role):
    ...
    def onDeactivate(self, subject):
        ...
        hasActivated -= { (subj, role) for (subj, role) in hasActivated if role.name == 'Consent_to_treatment' and role.pat == self.pat and role.org == self.org2 and role.cli == self.cli2 and role.spcty == self.spcty2 and other_consent_to_treatment_requests(subject, role.pat, role.org, role.cli, role.spcty) == 0 }
...
class Request_consent_to_group_treatment(Role):
    ...
    def onDeactivate(self, subject):
        ...
        hasActivated -= { (subj, role) for (subj, role) in hasActivated if role.name == 'Consent_to_group_treatment' and role.pat == self.pat and role.org == self.org and role.group == self.group and other_consent_to_group_treatment_requests(subject, role.pat, role.org, role.group) == 0 }
