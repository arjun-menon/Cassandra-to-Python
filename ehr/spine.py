...
class Spine_clinician(Role):
    ...
    def onDeactivate(self, subj):
        ...
        hasActivated -= { (s, r) for (s, r) in hasActivated if s == subj and r == Spine_emergency_clinician(self.org, Wildcard()) }
...
class Register_spine_admin(Role):
    ...
    def onDeactivate(self, subj):
        ...
        hasActivated -= { (s, r) for (s, r) in hasActivated if s == self.adm2 and r == Spine_admin() }
...
class Register_patient(Role):
    ...
    def onDeactivate(self, subj):
        ...
        hasActivated -= { (s, r) for (s, r) in hasActivated if s == Wildcard() and r == One_off_consent(self.pat) }
        ...
        hasActivated -= { (s, r) for (s, r) in hasActivated if s == Wildcard() and r == Request_consent_to_treatment(self.pat, Wildcard(), Wildcard(), Wildcard()) }
        
        # S2.4.7 -- deactive Request-consent-to-group-treatment(pat, org, group):
        hasActivated -= { (s, r) for (s, r) in hasActivated if s == Wildcard() and r == Request_consent_to_group_treatment(self.pat, Wildcard(), Wildcard()) }
        
        # S3.1.4 -- deactive Referrer(pat, org, cli2, spcty1):
        hasActivated -= { (s, r) for (s, r) in hasActivated if s == self.pat and r == Referrer(self.pat, Wildcard(), Wildcard(), Wildcard()) }
        
        # S3.2.4 -- deactive Spine-emergency-clinician(org, pat):
        hasActivated -= { (s, r) for (s, r) in hasActivated if s == Wildcard() and r == Spine_emergency_clinician(Wildcard(), self.pat) }
        
        # S4.1.5 -- deactive Concealed-by-spine-clinician(pat, ids, start, end):
        hasActivated -= { (s, r) for (s, r) in hasActivated if s == Wildcard() and r == Concealed_by_spine_clinician(self.pat, Wildcard(), Wildcard(), Wildcard()) }
        
        # S4.2.6 -- deactive Conceal-request(what, whom, start, end):
        hasActivated -= { (s, r) for (s, r) in hasActivated if s == Wildcard() and r == Conceal_request(Wildcard(), Wildcard(), Wildcard(), Wildcard()) and pi7_1(r.what) == self.pat }
        
        # S4.3.7 -- deactive Authenticated-express-consent(pat, cli):
        hasActivated -= { (s, r) for (s, r) in hasActivated if s == Wildcard() and r == Authenticated_express_consent(self.pat, Wildcard()) }
...
class Request_third_party_consent(Role):
    ...
    def onDeactivate(self, subj):
        ...
        hasActivated -= { (s, r) for (s, r) in hasActivated if s == self.x and r == Third_party() and other_third_party_consent_requests(subj, self.x) == 0 }
        
        # S2.2.16 -- deactive Third-party-consent(x, pat, id):
        hasActivated -= { (s, r) for (s, r) in hasActivated if s == self.x and r == Third_party_consent(self.x, self.pat, self.id) and other_third_party_consent_requests(subj, r.x) == 0 }
...
class Request_consent_to_treatment(Role):
    ...
    def onDeactivate(self, subj):
        ...
        hasActivated -= { (s, r) for (s, r) in hasActivated if s == Wildcard() and r == Consent_to_treatment(self.pat, self.org2, self.cli2, self.spcty2) and other_consent_to_treatment_requests(subj, r.pat, r.org, r.cli, r.spcty) == 0 }
...
class Request_consent_to_group_treatment(Role):
    ...
    def onDeactivate(self, subj):
        ...
        hasActivated -= { (s, r) for (s, r) in hasActivated if s == Wildcard() and r == Consent_to_group_treatment(self.pat, self.org, self.group) and other_consent_to_group_treatment_requests(subj, r.pat, r.org, r.group) == 0 }
...
class Conceal_request(Role):
    ...
    def onDeactivate(self, subj):
        ...
        hasActivated -= { (s, r) for (s, r) in hasActivated if s == Wildcard() and r == Concealed_by_spine_patient(self.what, self.who, self.start, self.end) }
